...
- Done: Desktop packaging v1 (Tauri v2) (`docs/guides/desktop-packaging.md`) — `desktop/src-tauri` embeds the existing `dist/` unchanged; `pnpm dev:desktop` / `pnpm build:desktop`; companion stays external (reached over 127.0.0.1); macOS arm64 produces a ~3 MB `.app` / ~2 MB `.dmg`. Code signing and cross-platform are deferred.
- Done: Embedded Companion sidecar (阶段四 · 方案 B 首期) — `companion/scripts/build-sidecar.mjs` compiles the companion into a single-file binary via `bun build --compile`; `companion/src/storage/sqliteDriver.ts` adapts SQLite per runtime (better-sqlite3 on Node, bun:sqlite in the compiled binary — the V8-bound addon can't load under Bun); the Rust shell (`lib.rs`) manages the sidecar lifecycle (reuse 19750 / spawn + `COMPANION_READY` handshake / ephemeral-port retry / kill on exit) with the shared `~/.gpt-image-studio` data dir; the webview auto-connects via `desktop_companion_info` (`src/services/desktopCompanion.ts`) with standalone fallback.
