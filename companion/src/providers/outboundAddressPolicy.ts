...
function createBlockedAddressList(): BlockList {
  ...
  const ipv4Subnets: Array<[string, number]> = [
    ...
    ["192.88.99.0", 24],
    ["192.168.0.0", 16],
    // 198.18.0.0/15（RFC 2544 基准测试段）已移除——该段被 Clash/Surge 等
    // 代理工具作为 fake-ip 默认地址段广泛使用，屏蔽会导致使用代理的用户
    // 无法下载 provider 返回的 CDN 图片。companion 是本地工具，SSRF 威胁
    // 模型较弱，放行该段对用户体验的收益大于安全风险。
    ["198.51.100.0", 24],
    ["203.0.113.0", 24],
    ["224.0.0.0", 4],
    ["240.0.0.0", 4],
