...
/**
 * 构造当前激活 provider 的状态摘要。
 *
 * 损坏探测由 /credentials 负责（它返 500+corrupt，Web 在 credError 通道展示）。
 * 这里用 getActiveCredential：它内部 catch CredentialStoreError 返 null，
 * 让调用方走「无凭据」正常分支。这样即使 /credentials 已经把损坏文件备份走，
 * /auth/status 仍能正常返 ready:false，不会因文件消失而异常。
 *
 * 抽取成独立函数供 /auth/status 路由和 admin 管理页的 /admin/api/status 复用，
 * 保证两者返回的 provider 状态完全一致。
 */
export function buildAuthStatus(): CompanionAuthStatus {
