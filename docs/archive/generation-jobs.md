...
- 第一阶段已完成：并发生成任务已落地，发送按钮不再被全局生成状态阻塞。
- 第二阶段已完成：对话级草稿已落地并持久化到 `conversationDrafts`（含文本、引用图、生成参数）。
- 第三阶段已完成：存在 pending job 时已注册 `beforeunload` 保护，并在任务结束后自动移除。
...
## 第四阶段：基于遮罩的图片局部编辑（已完成）

### 目标

实现 [遮罩局部编辑](../architecture/mask-editing.md) 中描述的 MVP，并把它接入新的对话级草稿模型。

### 接入要点

mask 已接入对话级草稿状态：

- 它绑定一张源图片（`editSourceImageId`）和对应 mask（`editMaskImageId`）。
- 源图片被移除或请求发送后，会清空编辑绑定。
...
当前实现中，mask 作为本地图片资产保存；产品决策为不做“正在绘制选区”的跨刷新恢复。
