...
- 链路更简单、更稳定（urlToB64 的 HTTPS、公网地址、大小限制和重试逻辑对豆包不触发）。
- 与 OpenAI 形状的 `data[0].b64_json` 契约天然吻合。

`urlToB64` 工具保留（GLM、Qwen 和 Wan 会使用），豆包 adapter 只是**不调用**它。若未来
改为 `url` + `urlToB64`，返回地址必须满足公网 HTTPS、PNG/JPEG/WebP 和 32 MiB 上限等
现行安全边界。
