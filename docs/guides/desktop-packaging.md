...
- **macOS 代码签名 / notarization**：需要 Apple Developer 账号。当前 `pnpm build:desktop` 产出的是未签名 app，首次打开需在「系统设置 → 隐私与安全性」里手动允许。签名后需注意 sidecar 二进制的 notarization 已知问题（[tauri#11992](https://github.com/tauri-apps/tauri/issues/11992)）。
- **Windows / Linux 跨平台构建**：当前只验证 macOS arm64（sidecar 构建脚本已预留 `SIDECAR_TRIPLE`，Windows 需补 `.exe` 后缀与交叉编译）。
- **自动更新（Tauri updater）**。
...
1. macOS 代码签名 + notarization（Apple Developer 账号；含 sidecar 二进制的 notarization 处理）。
2. Windows / Linux 构建 + 跨平台 CI。
3. Tauri updater 自动更新。
4. 评估是否需要把 IndexedDB 迁移到 Tauri 的文件系统存储（目前 IndexedDB 在 WKWebView 下持久化正常，暂无必要）。
5. 多 provider 前端管理 UI / keychain 凭据加密（阶段四启动时再决策）。
