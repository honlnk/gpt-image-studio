...
- mask 与源图通过 `editMaskImageId` / `editSourceImageId` 绑定。
- `EditImageInput` 增加可选 `mask`。
- 发送前校验 mask 图片 ID、尺寸和 PNG MIME 类型。
- 提交后或引用图变化后清空 mask。
- 增加编辑弹窗能力：画笔/橡皮/矩形/圆形/平移、撤销重做、缩放复位、软边。
- 输入区改为“编辑对”复合 tag（源图+mask 合并展示，关闭时联动移除）。

验收结果：

- 用户能从消息卡片“继续编辑”进入局部编辑模式（受“区域编辑”开关控制）。
- 完成涂抹后，原图和 mask 以复合引用 tag 展示并绑定在当前草稿。
- 发送图片编辑请求时携带 `mask`。
